...
Idents(int.sct) <- factor(int.sct$cell_type,
  levels = c(
    "Mesenchymal cells", "Stratified epithelium", "Immune cells",
...
prop.results <- propeller(
  clusters = meta$cell_type, sample = meta$sample,
...
prop.results <- propeller(
  clusters = meta$cell_type, sample = meta$orig.ident,
  group = meta$condition
)
...
stratified$cell_type <- factor(stratified$cell_type,
  levels = c(
    "TZ", "AC basal proximal", "AC proliferative basal proximal",
...
reactive.genes <- list(
  ...
  Epithelial_differentiation = c(
    "Krtdap", "Crabp2", "Fabp5", "Krt14", "Emp1", "Krt10",
...
wound_healing_signature <- list(
  ...
  phase_II = c(
    "Itga5", "Itga6", "Plau", "Plaur", "Ephb2", "Efnb1", "Cxcr4",
...
sel.genes <- c(
  "Krt13", "Gsta4", "Lgals3", "Tagln2", "Sult2b1",
...
ggsave(
  filename = paste0(results.dir, "dotplot_stratified_subtypes.png"),
  width = 10, height = 6, dpi = 300
)

i <- 1
VlnPlot(stratified.subset, paste0(names(sel.gene.sets)[i], i),
  group.by = "cell_type", sort = F
) + NoLegend() +
  geom_boxplot(aes(ymin = -Inf, ymax = Inf),
    width = 0.3, alpha = 0.2, color = "white"
  ) +
...
v1 <- VlnPlot(stratified.subset, paste0(names(sel.gene.sets)[i], i),
  ...
  stat_compare_means(
    label = "p.signif",
    comparisons = list(
      c("EMT-like cells", "Wounded cells"),
...
ggsave(
  filename = paste0(results.dir, "violin_stratified_subtypes.png"),
  ga1,
...
ggsave(
  filename = paste0(
    results.dir, "violin_wound.healing_phase.",
...
obj <- subset(obj, idents = c(
  "Cancerous cells", "Wounded cells",
...
barplot(props$Proportions,
  col = c("orange", "purple", "dark green"), legend = TRUE,
  ylab = "Proportions"
)
names(props)
...
propeller.anova(
  prop.list = props, design = design, coef = c(1, 2, 3, 4),
...
stratified.151 <- subset(stratified, cells = c(
  normal.cells,
  hyper.cells,
  dys.cells,
  cancer.cells
))
DimPlot(stratified.151,
  pt.size = 2, alpha = 0.5,
  split.by = "sample",
  group.by = "cell_type"
) +
...
immune.492 <- subset(immune, cells = c(
  normal.cells,
  hyper.cells,
  dys.cells,
  cancer.cells
...
umap_pl <- plotReducedDim(milo,
  dimred = "UMAP", colour_by = "condition", text_by = "cell_type",
...
ggsave(
  filename = paste0(results.dir, "milo_stratified_umap_nhoods.png"),
...
ggplot(da_results, aes(cell_type_fraction)) +
  geom_histogram(bins = 50)
...
ggsave(
  filename = paste0(results.dir, "milo_stratified_beeswarm.png"),
...
umap_pl + nh_graph_pl +
  plot_layout(guides = "collect")
ggsave(
  filename = paste0(results.dir, "milo_immune_umap_nhoods.png"),
  width = 12, height = 6, dpi = 300
)
...
plotDAbeeswarm(da_results, group.by = "celltype")
ggsave(
  filename = paste0(results.dir, "milo_immune_beeswarm.png"),
...
plot(reducedDims(sce)$UMAP,
  main = paste0("Trajectories from TZ Root Cluster"),
...
ggplot(df, aes(x = umapcca_1, y = umapcca_2, col = cl)) +
  ...
  scale_color_manual(values = cluster.colors) +
  geom_path(
    data = slingCurves(sce, as.df = TRUE) %>% arrange(Order),
...
ks.test(
  slingPseudotime(sce)[colData(sce)$condition == "A.Normal", 3],
  slingPseudotime(sce)[colData(sce)$condition == "B.Hyperplasia", 3]
) # p-value = 0.00236
ks.test(
  slingPseudotime(sce)[colData(sce)$condition == "B.Hyperplasia", 3],
  slingPseudotime(sce)[colData(sce)$condition == "C.Dysplasia", 3]
) # p-value = 4.618e-10
ks.test(
  slingPseudotime(sce)[colData(sce)$condition == "C.Dysplasia", 3],
  slingPseudotime(sce)[colData(sce)$condition == "D.Carcinoma", 3]
) # p-value < 2.2e-16
...
plotSmoothers(sce, assays(sce)$counts,
  gene = sel.smooth.gene, curvesCols = curvesCols,
  border = FALSE
) +
...
ggplot(meta, aes(x = slingPseudotime_3, y = GO_00973981)) +
  ...
  labs(
    title = "IL17 Activity Score Over Pseudotime",
...
meta_long <- meta %>%
  pivot_longer(
    cols = c(GO_00973981, GO_00050311, GO_00701021, GO_00050241),
    names_to = "GO_term",
    values_to = "value"
  )
