...
for (i in 1:length(cellchat.list)) {
  ...
  barplot(table(df.net$source),
    col = scPalette(length(levels(cellchat@idents))),
    las = 2, ylim = c(0, max.y),
    main = "Outgoing interactions per cell type"
  )
  barplot(table(df.net$target),
    col = scPalette(length(levels(cellchat@idents))), las = 2,
    las = 2, ylim = c(0, max.y),
...
netVisual_chord_gene(cellchat,
  sources.use = c(13, 15), targets.use = 1:6,
  slot.name = "net",
  directional = 1,
...
netVisual_bubble(cellchat,
  sources.use = sources.use, targets.use = targets.use, remove.isolate = FALSE,
  font.size = 12, font.size.title = 14, angle.x = 45, dot.size.min = 3, dot.size.max = 6,
  signaling = pathways.show
)
dev.off()
netVisual_bubble(cellchat,
  sources.use = targets.use, targets.use = sources.use, remove.isolate = FALSE,
  font.size = 12, font.size.title = 14, angle.x = 45, dot.size.min = 3, dot.size.max = 6,
  signaling = pathways.show
)
